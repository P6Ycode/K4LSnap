...
+ (NSDictionary *)containerRecordForBundleIdentifier:(NSString *)bundleIdentifier error:(NSError **)error {
    ...
    for (NSString *entry in entries) {
        ...
        NSString *identifier = metadata[@"MCMMetadataIdentifier"] ?: metadata[@"MCMMetadataInfo"][0];
        ...
        return @{ @"bundleIdentifier": bundleIdentifier,
                  ...
                  @"containerUUID": entry,
                  @"discoveredAt": @(NSDate.date.timeIntervalSince1970),
                  @"modificationDate": @([attributes.fileModificationDate timeIntervalSince1970]) };
    }
...
+ (BOOL)writeCachedRecord:(NSDictionary *)record error:(NSError **)error {
    ...
    NSString *directory = [K4LContainerCachePath() stringByDeletingLastPathComponent];
    ...
    NSDictionary *old = [self cachedRecord];
    BOOL ok = [record writeToFile:K4LContainerCachePath() atomically:YES];
    if (!ok && error) *error = [NSError errorWithDomain:@"com.p6ycode.k4lsnap.container" code:2 userInfo:@{NSLocalizedDescriptionKey:@"Unable to persist the container record"}];
    if (ok && ![old[@"containerPath"] isEqual:record[@"containerPath"]]) {
        CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(), (__bridge CFStringRef)K4LNotifyContainerChanged, NULL, NULL, true);
    }
    return ok;
}
