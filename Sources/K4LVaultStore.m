...
- (BOOL)open:(NSError **)error {
    ...
    dispatch_sync(self.queue, ^{
        ...
        if (sqlite3_open_v2(K4LDatabasePath().fileSystemRepresentation, &_db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE | SQLITE_OPEN_FULLMUTEX, NULL) != SQLITE_OK) {
            ok = NO; if (error) *error = [self errorWithCode:1 message:@(sqlite3_errmsg(self.db))]; return;
        }
        const char *schema =
        ...
        if (sqlite3_exec(self.db, schema, NULL, NULL, &message) != SQLITE_OK) {
            NSString *detail = message ? @(message) : @"Schema migration failed"; sqlite3_free(message);
            ok = NO; if (error) *error = [self errorWithCode:2 message:detail];
        }
    });
    return ok;
}
...
- (K4LVaultItem *)importFileAtURL:(NSURL *)sourceURL accountID:(NSString *)accountID friendID:(NSString *)friendID category:(NSString *)category error:(NSError **)error {
    ...
    dispatch_sync(self.queue, ^{
        ...
        if (sqlite3_prepare_v2(self.db, sql, -1, &stmt, NULL) != SQLITE_OK) { [NSFileManager.defaultManager removeItemAtPath:destination error:nil]; if (error) *error = [self errorWithCode:3 message:@(sqlite3_errmsg(self.db))]; return; }
        ...
        NSTimeInterval created = NSDate.date.timeIntervalSince1970; sqlite3_bind_double(stmt,7,created); sqlite3_bind_int64(stmt,8,[attributes fileSize]);
        ...
        else { [NSFileManager.defaultManager removeItemAtPath:destination error:nil]; if (error) *error = [self errorWithCode:4 message:@(sqlite3_errmsg(self.db))]; }
...
- (NSArray<K4LVaultItem *> *)itemsForAccount:(NSString *)accountID friendID:(NSString *)friendID category:(NSString *)category error:(NSError **)error {
    ...
    dispatch_sync(self.queue, ^{
        ...
        sqlite3_stmt *stmt=NULL; if (sqlite3_prepare_v2(self.db,sql.UTF8String,-1,&stmt,NULL)!=SQLITE_OK) { if(error)*error=[self errorWithCode:5 message:@(sqlite3_errmsg(self.db))]; return; }
        for (NSInteger i=0;i<bind.count;i++) sqlite3_bind_text(stmt,(int)i+1,[bind[i] UTF8String],-1,SQLITE_TRANSIENT);
        while (sqlite3_step(stmt)==SQLITE_ROW) { K4LVaultItem *item=[K4LVaultItem new]; item.identifier=@((const char*)sqlite3_column_text(stmt,0)); item.relativePath=@((const char*)sqlite3_column_text(stmt,1)); const unsigned char *a=sqlite3_column_text(stmt,2),*f=sqlite3_column_text(stmt,3),*c=sqlite3_column_text(stmt,4); item.accountID=a?@((const char*)a):nil; item.friendID=f?@((const char*)f):nil; item.category=c?@((const char*)c):nil; item.mediaType=@((const char*)sqlite3_column_text(stmt,5)); item.createdAt=sqlite3_column_double(stmt,6); item.byteSize=sqlite3_column_int64(stmt,7); [items addObject:item]; }
        sqlite3_finalize(stmt);
    });
    return items;
}
- (BOOL)deleteItem:(K4LVaultItem *)item error:(NSError **)error { if (![self open:error]) return NO; __block BOOL ok=NO; dispatch_sync(self.queue, ^{ sqlite3_stmt *stmt=NULL; if(sqlite3_prepare_v2(self.db,"DELETE FROM media_items WHERE id=?",-1,&stmt,NULL)==SQLITE_OK){ sqlite3_bind_text(stmt,1,item.identifier.UTF8String,-1,SQLITE_TRANSIENT); ok=sqlite3_step(stmt)==SQLITE_DONE; } sqlite3_finalize(stmt); if(ok)[NSFileManager.defaultManager removeItemAtPath:[K4LMediaDirectory() stringByAppendingPathComponent:item.relativePath] error:nil]; else if(error)*error=[self errorWithCode:6 message:@(sqlite3_errmsg(self.db))]; }); if(ok)K4LPostDarwinNotification(K4LNotifyVaultChanged); return ok; }
- (NSDictionary<NSString *,NSNumber *> *)categoryCountsForAccount:(NSString *)accountID error:(NSError **)error { if(![self open:error])return @{}; __block NSMutableDictionary *out=[NSMutableDictionary dictionary]; dispatch_sync(self.queue, ^{ const char *sql=accountID?"SELECT COALESCE(category,''),COUNT(*) FROM media_items WHERE account_id=? GROUP BY category":"SELECT COALESCE(category,''),COUNT(*) FROM media_items GROUP BY category"; sqlite3_stmt *stmt=NULL; if(sqlite3_prepare_v2(self.db,sql,-1,&stmt,NULL)!=SQLITE_OK){if(error)*error=[self errorWithCode:7 message:@(sqlite3_errmsg(self.db))];return;} if(accountID)sqlite3_bind_text(stmt,1,accountID.UTF8String,-1,SQLITE_TRANSIENT); while(sqlite3_step(stmt)==SQLITE_ROW) out[@((const char*)sqlite3_column_text(stmt,0))]=@(sqlite3_column_int64(stmt,1)); sqlite3_finalize(stmt); }); return out; }
@end
